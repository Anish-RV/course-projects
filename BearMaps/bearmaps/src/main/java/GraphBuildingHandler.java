...
public class GraphBuildingHandler extends DefaultHandler {
    ...
    private LinkedList<GraphDB.Node> wayNodes = new LinkedList<>();
    private HashMap<String, GraphDB.Node> nodes = new HashMap<>();
    private boolean rightway = false;
    ...
    public void startElement(String uri, String localName, String qName, Attributes attributes)
        ...
        if (qName.equals("node")) {
            ...
            /* DO: Use the above information to save a "node" to somewhere.
             * Hint: A graph-like structure would be nice. */
            GraphDB.Node n = new GraphDB.Node(attributes.getValue("id"),
                    attributes.getValue("lon"),
                    attributes.getValue("lat"), null);
            nodes.put(n.id, n);
            lastNode = n;  //Needed to save lastNode for 4th else if.
...
//
            ...
            wayNodes.addLast(nodes.get(nodeId));

            /* DO: Use the above id to make "possible" connections between the nodes in this way.
            ...
            GraphDB.Node node = lastNode;
            node.name = attributes.getValue("v");
            nodes.replace(node.id, node);

            /* DO: Create a location.
    ...
    public void endElement(String uri, String localName, String qName) throws SAXException {
        if (qName.equals("way")) {
            if (rightway) {
                GraphDB.Node previousNode = null;
                if (wayNodes.size() == 1) {
                    wayNodes = new LinkedList<>();
                    rightway = false;
                    return;
                }
                for (GraphDB.Node node: wayNodes) {
                    node.wayName = wayname;
                    g.addNode(node, previousNode);
                    previousNode = node;
                }
            }
            wayNodes = new LinkedList<>();
